...
import { KskDriverService } from '@/services/KskDriverService';
import { BOOLS } from './constants';

interface KskExcelImportModalProps {
  onClose: () => void;
  onImportSuccess: () => void;
}

export default function KskExcelImportModal({ onClose, onImportSuccess }: KskExcelImportModalProps) {
  ...
  const triggerImport = async () => {
    ...
    try {
      ...
      const mappedItems = json.map((row: any, idx) => {
        ...
        const defaultBools: Record<string, string> = {};
        BOOLS.forEach(b => { defaultBools[b] = 'Không'; });

        return {
          soPhieu: findVal(['số phiếu', 'so phieu', 'stt', 'so', 'số']) || String(idx + 1).padStart(3, '0'),
          hoTen: findVal(['họ tên', 'ho ten', 'họ và tên', 'ho va ten', 'tên bệnh nhân', 'ten benh nhan', 'bệnh nhân', 'benh nhan', 'tên', 'ten', 'name']),
          gioiTinh: findVal(['giới tính', 'gioi tinh', 'phái', 'phai']) || 'Nam',
          ngaySinh: findVal(['ngày sinh', 'ngay sinh', 'năm sinh', 'nam sinh']),
          tuoi: findVal(['tuổi', 'tuoi']),
          cccd: findVal(['cccd', 'cmnd', 'số cccd', 'so cccd']),
          capNgay: findVal(['cấp ngày', 'cap ngay']),
          capTai: findVal(['cấp tại', 'cap tai']),
          hoKhau: findVal(['hộ khẩu', 'ho khau', 'địa chỉ', 'dia chi']),
          hangLaiXe: findVal(['hạng lái xe', 'hang lai xe', 'hạng xe', 'hang xe']) || 'B2',
          ngayKham: new Date().toLocaleDateString('vi-VN'),
          ngayKetLuan: new Date().toLocaleDateString('vi-VN'),
          ketLuan: 'Đủ điều kiện sức khỏe lái xe theo hạng đăng ký.',
          nguoiKetLuan: 'BS. Nguyễn Văn A',
          bool: defaultBools
        };
      });
      ...
      if (validItems.length === 0) {
        throw new Error('Không tìm thấy dòng dữ liệu hợp lệ nào chứa đầy đủ Họ tên');
      }

      setImportStatus(`Đang import ${validItems.length} hồ sơ KSK...`);

      const result = await KskDriverService.importExcel(validItems);
