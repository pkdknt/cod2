...
export default function TiemChungPage() {
  ...
  const fetchSavedSchedules = async () => {
    try {
      ...
      const res = await fetch(`/api/tiem-chung?${qParams.toString()}`);
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      const response = await fetch('/api/tiem-chung', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id: activeEditId,
          patientCode: '',
          patientName: 'Tra cứu nhanh',
          phone: '',
          dob,
          gender,
          address: '',
  ...
  const handleDeleteSchedule = async (id: string) => {
    if (confirm('Bạn có chắc muốn xóa lịch tiêm của bệnh nhân này?')) {
      try {
        const response = await fetch(`/api/tiem-chung?id=${id}`, { method: 'DELETE' });
        const result = await response.json();
        if (!response.ok) throw new Error(result.message);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-black text-slate-800 tracking-tight flex items-center gap-2">
            💉 LỊCH TIÊM VẮC XIN NHƠN TÂM
          </h1>
          <p className="text-xs text-slate-500 font-semibold mt-1">
            PHÒNG KHÁM ĐA KHOA NHƠN TÂM • Tính toán và theo dõi phác đồ tiêm ngừa bệnh nhân.
          </p>
        </div>
      ...
      <div className="flex border-b border-slate-200 overflow-x-auto whitespace-nowrap bg-white p-2 rounded-2xl shadow-sm gap-1.5">
        ...
        >
          Danh sách khách hàng ({savedSchedules.length})
        </button>
      </div>
                    ...
                    <button 
                      onClick={async () => {
                         if(confirm(`Bạn có chắc muốn xóa ${selectedIds.length} hồ sơ đã chọn?`)) {
                            try {
                              const res = await fetch(`/api/tiem-chung?id=${selectedIds.join(',')}`, { method: 'DELETE' });
                              const result = await res.json();
